...
# Assignment 2. Error Handling: Add error handling to both functions to manage exceptions like connection errors or timeouts.
...
def get_example():
    ...
    try:
        response = requests.get(url)
        response.json()  # Raise an exception for HTTP errors
        if response.status_code == 200:
         print("GET request successful!")
        # Print response content
         print(response.json())
